...
def denoise_srgb(denoiser, data_folder, out_folder, roll_d=12, save_name=None):
    '''
    ...
    for i in range(50):
        ...
        for k in range(20):
            ...
            W = Inoisy_crop.shape[1]
            nlf = load_nlf(info, i)
            Idenoised_crop = denoiser(Inoisy_crop, roll_d=roll_d, nlf=nlf)
            ...
            Idenoised_crop = np.float32(Idenoised_crop).clip(0, 1)
            
            Image.fromarray(np.clip(Idenoised_crop*255, 0, 255).astype(np.uint8)).save('./results/DND/{}/{:0>3}.png'.format(save_name, i*20+k))
